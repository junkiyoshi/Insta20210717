...
void ofApp::draw() {
	...
	for (int radius = 300; radius <= 350; radius += 10) {
		...
		for (int i = 0; i < word.size(); i++) {
			...
			for (int outline_index = 0; outline_index < outline.size(); outline_index++) {
				...
				for (auto& vertex : vertices) {
					...
					auto angle_x = ofMap(ofNoise(noise_param.x, noise_location.x * 0.08, ofGetFrameNum() * 0.0035), 0, 1, -PI * 2, PI * 2);
					auto angle_y = ofMap(ofNoise(noise_param.x, noise_location.y * 0.08, ofGetFrameNum() * 0.0035), 0, 1, -PI * 2, PI * 2);
					auto angle_z = ofMap(ofNoise(noise_param.x, noise_location.z * 0.08, ofGetFrameNum() * 0.0035), 0, 1, -PI * 2, PI * 2);
